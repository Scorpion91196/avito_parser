...
global percents, good_proxy, pool, proxies
proxies = []
...
def write_csv(data):
    with open('video_cards.csv', 'a', encoding='utf8') as f:
        writer = csv.writer(f)
...
@second_thread
def start_video_cards_parser(signal, status_bar_signal, request_target_gui):
    time_start = time.time()
    global pool
    pool = ThreadPool(30)

    base_url = 'https://www.avito.ru/tomsk/tovary_dlya_kompyutera/komplektuyuschie/videokarty?'
    base_part = 'p='
    date_filter = '&s=104'
    price_max = '&pmax=6000'
    price_min = '&pmin=0'
    ...
    for i in range(1, 3):
        signal.emit('Парсинг страницы ' + str(i))
        status_bar_signal.emit(0)
        url_gen = base_url + base_part + str(i) + date_filter + price_max + price_min + request_target
        try:
            ...
            try:
                ...
                print(e)
                signal.emit("Что-то пошло не так, попробуйте перезапустить программу")
    ...
    signal.emit("Затрачено времени: " + str(int(time_end/60)) + ' минут ' + str(int(time_end % 60)) + ' секунд')
